  ...
  return (
    <>
      <h2 className="head-text">
        <span>Welcome</span> to My solid <span>Projects </span>Gallery
      </h2>

      <div className="app__works-filter">
        {["UI/UX", "webapp", "Mobile app", "React", "All"].map(
          (item, index) => (
            <div
              key={index}
              onClick={() => handleWorkFilter(item)}
              className="app__works-filter-item app_flex p-text ${activeFilter === item ? 'item-active' : ''}"
            >
              {item}
            </div>
          )
